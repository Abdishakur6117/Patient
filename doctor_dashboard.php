...
// Hubi in user-ku logged in yahay Doctor
if (!isset($_SESSION['user']) || $_SESSION['role'] != 'Doctor') {
    header("Location: login.php");
    exit();
}

$doctor_id = $_SESSION['doctor_id']; // Hubi inaad doctor_id sidan u kaydisay session

// Tirada balamaha doctor-ka
$result = $conn->query("SELECT COUNT(*) as count FROM appointments WHERE doctor_id = $doctor_id");
$totalAppointments = $result->fetch_assoc()['count'];

// Tirada visits doctor-ka
$result = $conn->query("
...
// Tirada prescriptions doctor-ka
$result = $conn->query("
    ...
    WHERE a.doctor_id = $doctor_id
");
$totalPrescriptions = $result->fetch_assoc()['count'];
