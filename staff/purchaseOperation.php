...
function create_purchase($conn) {
    ...
    if ($category_name) {
        ...
        $stmt = $conn->prepare("SELECT category_id FROM categories WHERE LOWER(name) = ? AND user_id = ?");
        $stmt->execute([strtolower($category_name), $user_id]);
        $existing_category = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($existing_category) {
            $category_id = $existing_category['category_id'];
        } else {
            // Insert new category using the same description and user_id
            $insertCat = $conn->prepare("
                INSERT INTO categories (name, description, user_id) 
                VALUES (?, ?, ?)
            ");
            $insertCat->execute([$category_name, $description, $user_id]);
            $category_id = $conn->lastInsertId();
        }
    }

    // 5. Check if product already exists
    $stmt = $conn->prepare("SELECT * FROM products WHERE LOWER(name) = ? AND user_id = ?");
    ...
    if ($product) {
        ...
        $update = $conn->prepare("UPDATE products SET quantity_in_stock = quantity_in_stock + ? WHERE product_id = ? AND user_id = ?");
        $update->execute([$quantity, $product_id, $user_id]);
    } else {
        // Product does not exist → insert new product with description & category
        $insertProduct = $conn->prepare("
            INSERT INTO products (name, quantity_in_stock, price, description, category_id, created_at, user_id) 
            VALUES (?, ?, ?, ?, ?, NOW(), ?)
        ");
        $insertProduct->execute([
            ...
            $category_id,
            $user_id        // Make sure user_id is inserted
        ]);
